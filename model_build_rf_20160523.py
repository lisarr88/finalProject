...
# Try model with breeds and breed groups:
#model_build1 = pd.concat([model_build, animal_type, sex_inds, sex_type_inds, color1, color1_type, akc_breed1], axis = 1) # 0.80758467
#model_build1 = model_build1.drop('color1_Ruddy',axis=1)
...
for depth in max_depth_range:
    rf_model = RandomForestClassifier(n_estimators=800, max_depth = depth,
                criterion = "gini", min_samples_split = 20, 
...
for split in best_samples_split:
    rf_model = RandomForestClassifier(n_estimators=800, max_depth = 20,
...
logloss = log_loss(y_test1,rf_predict_proba,eps=1e-15)
print(logloss)

# sensitivity:
conf_matrix = metrics.confusion_matrix(y_test1, rf_predict)
conf_matrix
...
crossval = cross_val_score(rf_model,X_train,y_train,scoring="accuracy",cv=5)
crossval


# Prune the model:
print(X_train1.shape)
print(rf_model_fit.transform(X_train1,threshold=0.01).shape)
X_important = rf_model_fit.transform(X_train1,threshold=0.01)
print(X_important)

X_imp_test = rf_model_fit.transform(X_test1,threshold=0.01)
print(X_imp_test.shape)

rf_model = RandomForestClassifier(n_estimators=800, max_depth = 15,
                criterion = "gini", min_samples_split = 20, 
                min_samples_leaf = 2, max_features = 'auto', n_jobs = 3)

rf_model_fit_imp = rf_model.fit(X_important, y_train1)

accuracy = rf_model_fit_imp.score(X_imp_test, y_test1)
print(accuracy)
...
ageplot = sns.countplot(data=shelter, x='age_category',hue='OutcomeType', order=['Baby', 'Young', 'Young Adult', 'Adult', 'Old'])
sns.axlabel("Age Category", "Count")


print(shelter.groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True))
nameplot = sns.countplot(data=shelter, x='name_flag',hue='OutcomeType')
sns.axlabel("", "Count")
nameplot.set(xticklabels=['No Name', 'Has Name'])

print(shelter[shelter['AnimalType']=='Dog'].groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True))
nameplot = sns.countplot(data=shelter[shelter['AnimalType']=='Dog'], x='name_flag',hue='OutcomeType',hue_order=['Adoption', 'Transfer', 'Return_to_owner', 'Euthanasia', 'Died'])
sns.axlabel("", "Count")
nameplot.set(xticklabels=['No Name', 'Has Name'])

print(shelter[shelter['AnimalType']=='Cat'].groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True))
nameplot = sns.countplot(data=shelter[shelter['AnimalType']=='Cat'], x='name_flag',hue='OutcomeType',hue_order=['Adoption', 'Transfer', 'Return_to_owner', 'Euthanasia', 'Died'])
sns.axlabel("", "Count")
nameplot.set(xticklabels=['No Name', 'Has Name'])


datap = shelter[shelter['AnimalType']=='Dog'].groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Died', 'Euthanasia', 'Return to owner', 'Transfer'])
_ = p.set_ylabel("Name Flag"), p.set_xlabel("Proportion of each outcome")

datap = shelter[shelter['AnimalType']=='Cat'].groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Died', 'Euthanasia', 'Return to owner', 'Transfer'])
_ = p.set_ylabel("Name Flag"), p.set_xlabel("Proportion of each outcome")
...
datap = shelter.groupby(['name_flag', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Died', 'Euthanasia', 'Return to owner', 'Transfer'])
_ = p.set_ylabel("Name Flag"), p.set_xlabel("Proportion of each outcome")
...
typeplot = sns.countplot(data=shelter, x='sex',hue='OutcomeType')
sns.axlabel("Sex", "Count")

typeplot = sns.countplot(data=shelter, x='AnimalType',hue='OutcomeType')
sns.axlabel("", "Count")
...
shelter['OutcomeNum'] = shelter['OutcomeType'].apply(outcome_to_num)
print(shelter['OutcomeNum'].value_counts())


print(shelter.groupby(['sex_type', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True))
datap = shelter.groupby(['sex_type', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Transfer', 'Return to owner', 'Euthanasia', 'Died'])
_ = p.set_ylabel("Sex Type"), p.set_xlabel("Proportion of each outcome")


datap = shelter.groupby(['AnimalType', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Transfer', 'Return to owner', 'Euthanasia', 'Died'])
_ = p.set_ylabel("Animal Type"), p.set_xlabel("Proportion of each outcome")



typeplot = sns.countplot(data=shelter[shelter['AnimalType']=='Dog'], x='breed_groups1',hue='OutcomeType')
sns.axlabel("", "Count")

datap = shelter[shelter['AnimalType']=='Dog'].groupby(['breed_groups1', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
#print(datap)
#print(datap.div(datap.sum(axis=1),  axis=0))
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Transfer', 'Return to owner', 'Euthanasia', 'Died'])
_ = p.set_ylabel("Breed Type"), p.set_xlabel("Proportion of each outcome")



typeplot = sns.countplot(data=shelter[shelter['AnimalType']=='Cat'], x='breed_groups1',hue='OutcomeType')
sns.axlabel("", "Count")

datap = shelter[shelter['AnimalType']=='Cat'].groupby(['breed_groups1', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
print(datap)
print(datap.div(datap.sum(axis=1),  axis=0))
_ = p.set_xticklabels("")
...
datap = shelter.groupby(['color1_nodesc', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
print(datap)
print(datap.div(datap.sum(axis=1),  axis=0))
_ = p.set_xticklabels("")
...
print(shelter.groupby(['pure_bred', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True))
colorplot = sns.countplot(data=shelter, x='pure_bred',hue='OutcomeType',hue_order=['Adoption', 'Transfer', 'Return_to_owner', 'Euthanasia', 'Died'])
sns.axlabel("", "Count")

datap = shelter.groupby(['pure_bred', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
print(datap)
print(datap.div(datap.sum(axis=1),  axis=0))
_ = p.set_xticklabels("")
_ = p.legend(['Adoption', 'Transfer', 'Return to owner', 'Euthanasia', 'Died'])
_ = p.set_ylabel("Pure Bred"), p.set_xlabel("Proportion of each outcome")


print(shelter.groupby(['Weekday', 'OutcomeType']).count()['AnimalID'].unstack().copy(deep=True))
colorplot = sns.countplot(data=shelter, x='Weekday',hue='OutcomeType',hue_order=['Adoption', 'Transfer', 'Return_to_owner', 'Euthanasia', 'Died'])
sns.axlabel("", "Count")

datap = shelter.groupby(['WeekdayNum', 'OutcomeNum']).count()['AnimalID'].unstack().copy(deep=True)
p = datap.div(datap.sum(axis=1),  axis=0).plot.barh(stacked=True, rot=0, width=0.5)
print(datap)
print(datap.div(datap.sum(axis=1),  axis=0))
_ = p.set_xticklabels("")
_ = p.set_yticklabels(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])
_ = p.legend(['Adoption', 'Transfer', 'Return to owner', 'Euthanasia', 'Died'])
_ = p.set_ylabel("Day of Week"), p.set_xlabel("Proportion of each outcome")
